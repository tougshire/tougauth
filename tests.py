...
class TestUserAndGroup(TestCase):
    ...
    def test_user_can_have_perm(self):
        usealpha = TougshireAuthUser.objects.get(username="usealpha")
        usealpha.user_permissions.add(
            Permission.objects.get(codename="view_tougshireauthuser")
        )
        self.assertTrue(usealpha.has_perm("tougshire_auth.view_tougshireauthuser"))

    def test_user_can_lose_perm(self):
        usealpha = TougshireAuthUser.objects.get(username="usealpha")
        usealpha.user_permissions.remove(
            Permission.objects.get(codename="view_tougshireauthuser")
        )
...
class TestAdmin(TestCase):
    def test_group_user_are_registered(self):
        for model in [TougshireAuthUser]:
            self.assertIs(
                True,
...
class TestUserName(TestCase):
    ...
    def test_bad_login(self):
        TougshireAuthUser.objects.create_user(
    ...
    def test_login(self):
        ...
        for template in response.templates:
            template_names.append(template.name)
        self.assertIn("tougshire_auth/profile_detail.html", template_names)
